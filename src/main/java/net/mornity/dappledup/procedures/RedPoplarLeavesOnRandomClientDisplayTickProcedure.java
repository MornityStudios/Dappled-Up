...
public class RedPoplarLeavesOnRandomClientDisplayTickProcedure {
		...
		if ((world.getBlockState(BlockPos.containing(x, y - 1, z))).getBlock() == Blocks.AIR || (world.getBlockState(BlockPos.containing(x, y - 1, z))).is(BlockTags.create(ResourceLocation.parse("dappled_up:leaf_bush")))) {
			if (Math.random() < 0.01) {
				if (world instanceof ServerLevel _level)
					_level.sendParticles((SimpleParticleType) (DappledUpModParticleTypes.RED_POPLAR_LEAF_PARTICLE.get()), (x + Mth.nextDouble(RandomSource.create(), 0, 1)), y, (z + Mth.nextDouble(RandomSource.create(), 0, 1)), 1, 0, 0, 0, 0);
			}
		}
	}
}